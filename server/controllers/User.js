...
export class User{
    ...
    static register = async (req, res, next) => {
        try{
            ...
            try{
                ...
                if(await findUserByEmail(email)) return res.status(409).json(
                    {status:409, error: `User with this email already exist`})
                if(await findUserByUsername(username)) return res.status(409).json(
                    {status:409, error: `Username already taken`
                })
                ...
                await db.wallet.create({user_model: user.id}, {transaction: t})
                await t.commit();
    ...
    static update = async (req, res) => {
        try {
            ...
            if(!userExist) return res.status(404).json({status:404, error:"User does not exist"})
            await db.users.update(req.body,{where:{id: userId}});
            const result = await db.users.findOne({where:{id: userId}});
            return res.status(202).json({message:"User updated successful", data: result})
        } catch (error) {
            return res.status(200).json({status: 500, error:error.message, stack:error.stack})
